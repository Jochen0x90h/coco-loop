...
GuiRotaryKnob::Result GuiRotaryKnob::update(Gui &gui) {
    ...
    int offset = int(this->button) * 4; // inner select color depending on button press
    float angle = float(this->value & 0xffff) / (PERIOD * this->increments);
    resize(gui.draw<Wheel>(this->innerRadius, outerColor, innerColor + offset, this->increments, angle));
...
void GuiRotaryKnob::touch(bool first, float x, float y) {
    ...
    // check if on inner area
    bool inner = this->haveCenterButton && length(p) < this->innerRadius;
    if (first) {
        // check if button (inner circle) was hit
        this->button = inner;
    } else if (!inner) {
        // last vector on widget
        float2 q = this->lastPosition;

        // calc angle (in rad) between vectors (approximation, is actually sin(angle))
        float d = (p.y * q.x - p.x * q.y) / (length(p) * length(q));
...
/*
    float ax = x - 0.5f;
    float ay = y - 0.5f;
    bool inner = this->haveButton && std::sqrt(ax * ax + ay * ay) < this->innerRadius;
    if (first) {
        // check if button (inner circle) was hit
        this->button = inner;
    } else if (!inner) {
