...
void Gui::drawText(const TextureFont &font, const float2 &position, const float2 &scale, String text) {
    ...
    for (auto glyph : font.glyphRange(text)) {
        if (glyph.size.y == 0) {
            ...
            float u1 = uScale * glyph.location.x;
            float v1 = vScale * glyph.location.y;
...
void Gui::drawText(const TextureFont &font, int id, const float2 &scale, String text) {
